...
After running our model, we save individual RDS objects for each iteration of the model that we run. We stores these objects in our output folder. In order to analyze the results, we have to combine these individual results table into a cumulative dataframe for plotting. It is also important to make sure the data is in tidy format. 
...
We can bind our data tables into one cumulative dataframe. We can then calculate model performance metrics for multiclass classifers on our predictions. There are 3 ways to measure multiclass performance.

+ **macro averaging** - reduce multiclass predictions to multiple sets of binary (all vs one) predictions, and calcualtes the corresponding metric for each of the binary cases. Each class gets equal weight (bad for imbalanced)

+ **weighted macro average** - same as previous except metrics are weighed by existing proportion in data.

+ **micro average** - treats the entire dataset as an aggregate results, and calculates 1 metric rather than k metrics averaged together. For example; 
...
multi_metric <- metric_set(accuracy, bal_accuracy, f_meas, kap, mcc,  ppv, precision, recall, sens, spec)
...
results_tune <- results_tune_raw %>%
  ...
  mutate(.micro = future_map(.predictions, multi_metric, estimate = ".pred_class", truth = "response", estimator = "micro")) %>% #calculate micro weighted stats
	...
	mutate(.custom2 =future_map(.predictions, custom_evaluate)) #use a custom function to predict for each individual class
