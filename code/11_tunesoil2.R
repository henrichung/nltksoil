...
train_folds <- vfold_cv(test, v = nrepeats, strata = response)
#check tune results
...
final_results <- list(tune_results, train_results, test_results, final_model)
names(final_results) <- c("tune", "train", "test", "model")
