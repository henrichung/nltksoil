...
custom_transform <- function(input, transform_embedding){
    input <- select(input, any_of(rownames(transform_embedding)))
    transform_embedding <- subset(transform_embedding, rownames(transform_embedding) %in% colnames(input))
...
step_prev_filter<-function(
  ...
  skip = FALSE, 
  columns = NULL,
  options = list(prevalence= 0, names = TRUE),
...
custom_summarize_taxa <- function(input, taxa_level, otu_key){
  ...
  #names <- janitor::make_clean_names(res[["taxonomy"]])
  res <- res %>%
    #select(-c("taxonomy")) %>%
