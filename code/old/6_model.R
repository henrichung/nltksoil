...
#read in RDS data
otu_test <- readRDS(paste(dataFolder, otuFilename_test, sep = "")) %>%
  select(-c("#OTU ID"))
otu_train_raw <- readRDS(paste(dataFolder, otuFilename_train, sep = ""))%>%
  select(-c("#OTU ID"))
set.seed(123)
n <- sample(1:(ncol(otu_train_raw)-1), 2000)
otu_setaside <- otu_train_raw[,..n]
otu_train <- otu_train_raw %>%
  select(!contains(colnames(otu_test[,1:(ncol(otu_test)-1)]))) %>%
...
response = "envo_biome_2"
taxa_space <- c("genus") #, "family" , "order", "class", "phylum", "species")
...
filtered_train <- custom_prev_filter(otu_train, prevalence)
filtered_test <- otu_train[otu_test$taxonomy %in% filtered_train$taxonomy]
...
custom_summarize_taxa <- function(x, y){
  ...
  if(y == "species"){
      ...
      res <- res %>%
        ...
        mutate(speces = paste("s__", 1:nrow(res), sep = ""))
